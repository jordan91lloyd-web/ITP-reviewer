    ...
    <div className="max-w-6xl mx-auto px-6 py-6 space-y-6">
      ...
      {projectId && baselineDocs.some((d) => d.status === "processed") && (
        <div style={{ marginBottom: 24 }}>
          <DocCompareSection company_id={company_id} project_id={projectId} project_name={projects.find((p) => String(p.id) === projectId)?.name ?? ""} />
        </div>
      )}
...
function DocCompareSection({ company_id, project_id, project_name }: { company_id: string; project_id: string; project_name: string }) {
  const [file, setFile] = useState<File | null>(null);
  const [isComparing, setIsComparing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [results, setResults] = useState<{ document_name: string; changes: DocCompareResult[]; by_risk: Record<string, number> } | null>(null);
  const [isDragging, setIsDragging] = useState(false);
  const [expanded, setExpanded] = useState(true);
  const inputRef = useRef<HTMLInputElement>(null);

  // Load previous document comparisons from results API
  const [pastComparisons, setPastComparisons] = useState<Array<{ scan_id: string; document_name: string; changes: DocCompareResult[]; created_at: string }>>([]);
  const [expandedPast, setExpandedPast] = useState<string | null>(null);

  useEffect(() => {
    // Fetch document-type changes from results
    fetch(`/api/drawing-changes/results?company_id=${company_id}&project_id=${project_id}&all=true`)
      .then((r) => r.json())
      .then((data) => {
        if (data.changes) {
          // Filter to Document discipline and group by scan_id
          const docChanges = (data.changes as Array<DocCompareResult & { discipline: string; scan_id: string; drawing_number: string; created_at: string }>)
            .filter((c) => c.discipline === "Document");
          const byScan = new Map<string, { document_name: string; changes: DocCompareResult[]; created_at: string }>();
          for (const c of docChanges) {
            if (!byScan.has(c.scan_id)) byScan.set(c.scan_id, { document_name: c.drawing_number, changes: [], created_at: c.created_at });
            byScan.get(c.scan_id)!.changes.push(c);
          }
          setPastComparisons([...byScan.entries()].map(([scan_id, v]) => ({ scan_id, ...v })).sort((a, b) => b.created_at.localeCompare(a.created_at)));
        }
      })
      .catch(() => {});
  ...
  const handleFile = useCallback((f: File) => {
    setError(null); setResults(null); setFile(f);
  }, []);

  const handleCompare = async () => {
    if (!file) return;
    setIsComparing(true); setError(null); setResults(null);
    try {
      const fd = new FormData();
      fd.append("file", file);
      fd.append("company_id", company_id);
    ...
    } catch { setError("Comparison timed out or failed."); }
  ...
  return (
    <div style={{ borderRadius: 8, border: "1px solid var(--hp-border)", overflow: "hidden" }}>
      <div
        ...
        style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", backgroundColor: "var(--hp-bg)", cursor: "pointer", userSelect: "none" }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {expanded ? <ChevronDown size={16} style={{ color: "var(--hp-text-muted)" }} /> : <ChevronRight size={16} style={{ color: "var(--hp-text-muted)" }} />}
      ...
      {expanded && (
        <div style={{ padding: 16, borderTop: "1px solid var(--hp-border)" }}>
          ...
          <div
            ...
            style={{
              cursor: "pointer", borderRadius: 8, border: "2px dashed", padding: 24, textAlign: "center",
          ...
          >
            ...
            {file ? (
              <div>
                ...
                <p style={{ fontSize: 11, marginTop: 4, color: "var(--hp-text-muted)" }}>{(file.size / 1024 / 1024).toFixed(1)} MB</p>
              </div>
            ) : (
          ...
          {error && (
            <div style={{ marginTop: 12, borderRadius: 8, padding: "10px 14px", fontSize: 13, backgroundColor: "#fef2f2", border: "1px solid #fecaca", color: "#991b1b" }}>{error}</div>
          )}

          <button onClick={handleCompare} disabled={!file || isComparing}
            style={{ marginTop: 12, width: "100%", borderRadius: 8, backgroundColor: "var(--hp-warm-800)", padding: "10px 16px", fontSize: 13, fontWeight: 600, color: "#fff", border: "none", cursor: file && !isComparing ? "pointer" : "not-allowed", opacity: !file || isComparing ? 0.4 : 1 }}>
          ...
          {results && (
            <div style={{ marginTop: 16 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: "var(--hp-warm-800)" }}>{results.document_name} — {results.changes.length} change{results.changes.length !== 1 ? "s" : ""} found</span>
                <div style={{ display: "flex", gap: 8 }}>
                  {(results.by_risk.likely_variation ?? 0) > 0 && <span style={{ fontSize: 10, fontWeight: 500, borderRadius: 999, padding: "1px 8px", backgroundColor: "var(--hp-critical-bg)", color: "var(--hp-critical)" }}>{results.by_risk.likely_variation} variation{results.by_risk.likely_variation !== 1 ? "s" : ""}</span>}
                  {(results.by_risk.within_scope ?? 0) > 0 && <span style={{ fontSize: 10, fontWeight: 500, borderRadius: 999, padding: "1px 8px", backgroundColor: "var(--hp-compliant-bg)", color: "var(--hp-compliant)" }}>{results.by_risk.within_scope} in scope</span>}
                  {(results.by_risk.unclear ?? 0) > 0 && <span style={{ fontSize: 10, fontWeight: 500, borderRadius: 999, padding: "1px 8px", backgroundColor: "var(--hp-significant-bg)", color: "var(--hp-significant)" }}>{results.by_risk.unclear} unclear</span>}
                </div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {results.changes.map((c, i) => (
                  <div key={i} style={{ borderRadius: 6, padding: "8px 12px", backgroundColor: "var(--hp-surface)", border: "1px solid var(--hp-border)", fontSize: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      <span style={{ fontSize: 10, fontWeight: 500, borderRadius: 999, padding: "1px 8px", backgroundColor: riskBg(c.variation_risk), color: riskColor(c.variation_risk) }}>{riskLabel(c.variation_risk)}</span>
                      <span style={{ fontSize: 10, fontWeight: 500, color: sevColor(c.severity), textTransform: "uppercase" }}>{c.severity}</span>
                      <span style={{ fontSize: 10, color: "var(--hp-text-muted)" }}>{c.change_type}</span>
                      {c.location_on_drawing && <span style={{ fontSize: 10, color: "var(--hp-text-muted)" }}>{c.location_on_drawing}</span>}
                    </div>
                    <div style={{ color: "var(--hp-warm-800)", fontWeight: 500 }}>{c.description}</div>
                    {c.variation_note && <div style={{ marginTop: 4, color: "var(--hp-text-secondary)", fontSize: 11 }}>{c.variation_note}</div>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {pastComparisons.length > 0 && (
            <div style={{ marginTop: 16, borderTop: "1px solid var(--hp-border)", paddingTop: 12 }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: "var(--hp-text-muted)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>Previous comparisons</div>
              {pastComparisons.map((pc) => (
                <div key={pc.scan_id} style={{ marginBottom: 4 }}>
                  ...
                  >
                    {expandedPast === pc.scan_id ? <ChevronDown size={12} style={{ color: "var(--hp-text-muted)" }} /> : <ChevronRight size={12} style={{ color: "var(--hp-text-muted)" }} />}
                    <span style={{ fontWeight: 500, color: "var(--hp-warm-800)" }}>{pc.document_name}</span>
                    <span style={{ color: "var(--hp-text-muted)" }}>{pc.changes.length} change{pc.changes.length !== 1 ? "s" : ""}</span>
                    <span style={{ color: "var(--hp-text-muted)", fontSize: 10 }}>{pc.created_at.slice(0, 10)}</span>
                  </div>
                  {expandedPast === pc.scan_id && (
                    <div style={{ paddingLeft: 20, display: "flex", flexDirection: "column", gap: 4, marginBottom: 8 }}>
                      {pc.changes.map((c, i) => (
                        <div key={i} style={{ borderRadius: 6, padding: "6px 10px", backgroundColor: "var(--hp-surface)", border: "1px solid var(--hp-border)", fontSize: 11 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                            <span style={{ fontSize: 10, fontWeight: 500, borderRadius: 999, padding: "1px 6px", backgroundColor: riskBg(c.variation_risk), color: riskColor(c.variation_risk) }}>{riskLabel(c.variation_risk)}</span>
                            <span style={{ fontSize: 10, color: "var(--hp-text-muted)" }}>{c.change_type}</span>
                          </div>
                          <div style={{ color: "var(--hp-warm-800)" }}>{c.description}</div>
                          {c.variation_note && <div style={{ marginTop: 2, color: "var(--hp-text-secondary)", fontSize: 10 }}>{c.variation_note}</div>}
                        </div>
                      ))}
                    </div>
                  )}
