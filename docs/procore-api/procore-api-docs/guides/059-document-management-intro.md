# Document Management Integration Overview

Source: https://developers.procore.com/documentation/document-management-intro
...
- **Overview** (this page)
- [Technical Guide](060-document-management-technical-guide.md)
...
- **Authentication** - Leverage Procore's OAuth 2.0 framework to authenticate requests. See [Choosing an OAuth 2.0 Grant Type](014-oauth-choose-grant-type.md) for additional information.
...
> **The steps below are a high-level workflow overview.** For detailed step-by-step implementation instructions, see [Document Management Technical Guide](060-document-management-technical-guide.md).
...
Note: the update API exposes a batch update endpoint for updating multiple uploads; batch updates may return HTTP 207 with partial success/failure results. See [Technical Guide](060-document-management-technical-guide.md) for complete examples and error-handling patterns.
...
Note: Submissions may fail due to workflow conflicts; the API provides conflict-resolution parameters to handle these cases. See [Technical Guide](060-document-management-technical-guide.md) for complete examples and error-handling patterns.
...
- **API Rate Limits** - Usage of Procore's API is subject to rate limits. The rate limit resets every hour. See [Rate Limiting](015-rate-limiting.md) to learn how to reduce the possibility of exceeding the rate limit.
