...
Source: https://developers.procore.com/documentation/oauth-client-credentials
...
The Procore API implementation of the Client Credentials flow relies on the use of a [_Developer Managed Service Account (DMSA)_](017-developer-managed-service-accounts.md).
...
With a DMSA installed in Procore, request an access token by POSTing to the `/oauth/token` endpoint with `grant_type=client_credentials`. For full parameter details, see [Authentication Endpoints](064-oauth-endpoints.md).
...
For additional information on MPR, see [Working with Multiple Procore Regions](008-tutorial-mpz.md).
