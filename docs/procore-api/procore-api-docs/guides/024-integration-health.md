...
_Monitor your app's health and compliance against Procore's best practices — what each observation means, why it matters, and how to resolve unhealthy states._

Source: https://developers.procore.com/documentation/integration-health
...
For a complete reference of HTTP status codes, see the [Error Code Reference](073-error-reference.md). For broader debugging guidance, see [Troubleshooting](026-troubleshooting.md).
...
- **API Call Activity Report** — a CSV download covering the last 30 days of production API calls for your app. Use this to investigate any observation flagged on the page. See [API Call Activity Report](025-api-activity-export.md) for the full column reference.
...
- **Before changelog deprecations land:** Subscribe to the [API Changelog](078-overview.md) and check for **Deprecated Endpoint Usage** as soon as a new deprecation is announced — not after sunset.
...
3. See [Error Code Reference — 403 Forbidden](073-error-reference.md#client-errors-4xx) and [App Install & Setup Overview](007-building-apps-install-arch.md).
...
3. Subscribe to relevant [webhook events](016-webhooks.md) to invalidate cached IDs when records are deleted upstream.
...
2. Reduce request volume:
   - Replace polling with [webhooks](016-webhooks.md) where supported.
   - Implement **exponential backoff with jitter** on retries — never retry a 429 immediately.
   - Honor the `Retry-After` response header.
   - Use [Sync Actions](072-using-sync-actions.md) to batch creates and updates instead of issuing one call per record.
   - Cache responses where the data does not need to be real-time.
3. See [Rate Limiting](015-rate-limiting.md) for thresholds and patterns.
...
3. See [Troubleshooting — Authentication & Token Issues](026-troubleshooting.md) and [OAuth 2.0 Authorization Code Grant](062-oauth-auth-grant-flow.md).
...
3. See [Handling Procore Regions (MPZ)](008-tutorial-mpz.md) to understand when and why the header is required.
...
**How to resolve:**
1. Review the [API Changelog](078-overview.md) and the [API Lifecycle](010-rest-api-lifecycle.md) page to identify supported replacement endpoints.
...
2. Cross-reference each route against the [REST API Reference](https://developers.procore.com/reference/rest/v1/docs/rest-api-overview). Any route that does not appear there is private.
...
The **API Call Activity Report** is the CSV download at the top of the Integration Health tab. It contains the last 30 days of production API calls for your app — endpoints, status codes, HTTP methods, and daily counts. See [API Call Activity Report](025-api-activity-export.md) for the full column reference and tips for filtering it.

### Does my app status affect my Marketplace listing?

Marketplace requirements may consider integration health and compliance signals as part of ongoing eligibility. See [Marketplace Requirements](031-marketplace-requirements.md) for details on what is reviewed and how often.
...
- [Handling Procore Regions (MPZ)](008-tutorial-mpz.md)
- [API Usage Guidelines](011-api-usage-guidelines.md)
- [API Lifecycle](010-rest-api-lifecycle.md)
- [API Call Activity Report](025-api-activity-export.md)
