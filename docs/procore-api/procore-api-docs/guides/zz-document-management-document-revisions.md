...
Source: https://developers.procore.com/documentation/document-management-document-revisions

---

## Overview

The [Document Management Technical Guide](060-document-management-technical-guide.md) covers the eight-step submit flow: initialize an upload, put the file in storage, enrich metadata, and submit. After Step 8, the upload is consumed and the permanent record is a **Document Revision** in a **Document Container**.
...
Use this endpoint when you already have revision IDs — for example the `ids` array returned by [Step 8: Submit Document Uploads as Revisions](060-document-management-technical-guide.md#step-8-submit-document-uploads-as-revisions), or the IDs from a `view=ids_only` list pass.
...
See [Introduction to Webhooks](016-webhooks.md) and [Using the Webhooks API](021-webhooks-api.md) for setup, retry, and delivery-monitoring details.
...
401 `INVALID_TOKEN`, 403 `ACCESS_DENIED`, and 500 `INTERNAL_ERROR` apply across Document Management V2 endpoints. Handle them globally — see [Standard API Errors](060-document-management-technical-guide.md#standard-api-errors) in the Technical Guide. Rate limiting is enforced at the API gateway; see [Rate Limiting](015-rate-limiting.md).
...
- [Filtering and Sorting on List Actions](068-filtering-on-list-actions.md)
